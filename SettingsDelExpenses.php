<?php
	...
	try
	{
		...
		{
			$result = $connection->query("SELECT exp.Value, exp.Date, exp.Comment, exp.ID, (SELECT Name FROM expcathegories WHERE exp.CathegoryID = CathegoryID ) cathName, (SELECT Name FROM paymentmethod WHERE exp.PayMetID = PayMetID) paymName FROM expenses exp WHERE exp.UserID = ".$_SESSION['userId']);
			$connection->close();
			  ...
			  <tr>
				...
				<?php
					...
					while($row = mysqli_fetch_array($result)) {
						...
						echo "<td><center><Strong>".$row['paymName']."</Strong></center></td>";
